...
> 状态：草案 v0.1  
> 依赖：[UX 索引](./README.md) · [产品文案](./copy.md) · [设计规范](./design-system.md) · [多平台](./multi-platform.md)  
> 参考：[Flutter Internationalization](https://docs.flutter.dev/ui/accessibility-and-internationalization/internationalization)

本文约定 ARB 工程、区域格式、RTL 与翻译工作流。**界面写什么、语气如何**见 [产品文案规范](./copy.md)。NextPili 面向中文用户为主，但 **工程从第一天按可本地化搭建**，避免硬编码中文渗透业务层。
...
语气、句式、场景范文与禁词见 **[产品文案规范](./copy.md)**（对齐 Apple HIG Writing）。本节只约定工程落点。

...
| 决策 | 值 |
|------|-----|
| 视觉语言 | Liquid Glass（内容不透明 + chrome 玻璃；桌面外壳不透明） |
| 实现库 | `liquid_glass_widgets` `^0.22.1` · `window_manager` `^0.5.2` |
| 桌面外壳 | 不透明窗口；Rail / 桌面紧凑底栏走 `ChromeSurface`（`color.bg.elevated` + 边线）；内容用 canvas |

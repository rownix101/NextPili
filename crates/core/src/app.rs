...
use http::{BiliClient, ClientConfig};
...
impl CoreApp {
    ...
    pub fn bootstrap(config: BootstrapParams) -> Result<Arc<CoreApp>, AppError> {
        ...
        let store = Store::open(&config.data_dir).map_err(AppError::from)?;
        ...
        tracing::info!(data_dir = %config.data_dir, "NextPili core bootstrapped");
        Ok(app)
    }
}
